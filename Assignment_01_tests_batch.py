import numpy as np
from Gunasekharan_01_01 import multi_layer_nn
...
def test_can_fit_data_test():
    ...
    assert err[100] < 0.15
    assert err[999] < 0.1
    # assert abs(err[999] - 0.10182781417045624) < 1e-5
...
def test_check_output_shape():
    ...
    [W, err, Out] = multi_layer_nn(X_train, Y_train, X_test, Y_test, [
                                   100, 1], alpha=1e-9, epochs=1, h=1e-8, seed=2)
    print(Out)
    assert Out[0].shape == Y_test.shape
...
def test_check_output_values():
    ...
    [W, err, Out] = multi_layer_nn(X_train, Y_train, X_test, Y_test, [
                                   2, 2], alpha=0.35, epochs=1, h=1e-8, seed=1234)
    print(Out)
    ...
    assert np.allclose(Out, expected_Out, atol=1)
...
def test_check_weight_update():
    ...
    assert np.allclose(delta1, correct_delta1, atol=1e-2)
    assert np.allclose(delta2, correct_delta2, atol=1e-2)
